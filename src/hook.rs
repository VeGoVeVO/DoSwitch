...
unsafe extern "system" fn callback(code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    ...
    match known {
        ...
        None => {
            ...
            // above; let this one key through, and by the next keystroke
            // the answer is in the cache.
    ...
    match app::account_for(bind) {
        Some(character) => {
            ...
            // If the post fails the key has been taken from the game and
